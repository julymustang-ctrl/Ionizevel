    ...
    <form action="{{ route('admin.articles.update', $article->id_article) }}" method="POST">
        ...
        <div class="panel">
            ...
            <div class="panel-body">
                ...
                <div class="form-group">
                    <label>
                        <input type="checkbox" name="indexed" value="1" {{ $article->indexed ? 'checked' : '' }}> Arama motorlarında indekslensin
                    </label>
                </div>

                <div class="form-group">
                    <label>
                        <input type="checkbox" name="comment_allow" value="1" {{ $article->comment_allow ? 'checked' : '' }}> Yorumlara izin ver
                    </label>
                </div>
            </div>
        </div>
                ...
                @foreach($languages as $index => $lang)
                    ...
                    <div class="lang-content" id="lang-{{ $lang->lang }}" style="{{ $index !== 0 ? 'display:none;' : '' }}">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="content_{{ $lang->lang }}" class="form-control" rows="12">{{ old("content_{$lang->lang}", $translation->content ?? '') }}</textarea>
                        </div>
